...
function InputField({ label, icon, type, ...props }: { label: string; icon: React.ReactNode } & React.InputHTMLAttributes<HTMLInputElement>) {
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        <input {...props} type={isPassword && revealed ? "text" : type} className={`w-full rounded-2xl border-2 border-slate-100 bg-slate-50 py-3.5 pl-12 ${isPassword ? "pr-12" : "pr-4"} text-sm font-medium text-slate-900 placeholder-slate-300 outline-none transition-all focus:border-blue-400/50 focus:bg-white focus:ring-4 focus:ring-blue-500/5`} />
        {isPassword && (
          <button type="button" tabIndex={-1} onClick={() => setRevealed((v) => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 transition-colors hover:text-slate-500 focus:outline-none">
            {revealed
