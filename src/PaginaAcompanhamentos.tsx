...
import { buscarTodosAcompanhamentos, buscarAcompanhamentos, buscarPacientes, excluirAcompanhamento, buildFiltroRole } from "./pocketbase";
...
function renderGrupos(p: Paciente) {
  ...
  if (p.menor_de_2_anos) itens.push({ label: "≤2a", className: getCoresCategoria("crianca") });
  if (itens.length === 0) return null;
  return (
    <div className="flex flex-wrap justify-center gap-1">
      {itens.map((item) => (
        <span key={item.label} className={`inline-flex items-center rounded-full px-2.5 py-[1px] text-[9px] font-bold uppercase tracking-wider ${item.className}`}>
          {item.label}
        </span>
      ))}
    </div>
...
export default function PaginaAcompanhamentos({ selectedPacienteId }: { selectedPacienteId?: string | null }) {
  ...
  const pacsParaEquipes = filtroUnidadeDraft !== "todas" ? Object.values(pacMap).filter(p => p.unidade === filtroUnidadeDraft) : Object.values(pacMap);
  const equipes = [...new Set(pacsParaEquipes.map(p => p.equipe).filter(Boolean))].sort();
  const pacsParaMicro = pacsParaEquipes.filter(p => filtroEquipeDraft === "todas" || p.equipe === filtroEquipeDraft);
  const microareas = [...new Set(pacsParaMicro.map(p => p.microarea).filter(Boolean))].sort();
  ...
  useEffect(() => {
    ...
    async function carregar() {
      try {
        ...
        const pacsPromise = buscarPacientes({ perPage: 500, filter: filtroRole ?? undefined });
        ...
        const acompsFiltrados = selectedPacienteId
          ? acomps
          : acomps.filter((a) => pacsPermitidos.has(a.paciente_id));
        setAcompanhamentos(acompsFiltrados);
      } catch { /* ignore */ }
  ...
  const filtradosComGrupo = (() => {
    ...
    if (filtro !== "todos") {
      resultado = resultado.filter((a) => {
        ...
        if (filtro === "crianca") return pac.menor_de_2_anos === true;
      ...
      <div className="relative overflow-hidden rounded-b-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-cyan-950 px-5 py-5 sm:px-6 shadow-xl shadow-slate-900/30">
        ...
        <div className="relative mx-auto flex max-w-[1380px] flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-xl font-black tracking-tight text-white sm:text-2xl whitespace-nowrap">
              ACOMP.<br/><span className="text-cyan-300 font-bold">Registrados</span>
            </h1>
          </div>
