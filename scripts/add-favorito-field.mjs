/**
 * Script para CRIAR a coleção "painelsorriso53_favoritos" no PocketBase.
 *
 * Coleção: painelsorriso53_favoritos
 *   ├── usuario_id  (text)  → ID do usuário que favoritou
 *   └── paciente_id (text)  → ID do paciente favoritado
 *
 * Cada registro é único por par (usuario_id, paciente_id).
 *
 * Uso:
 *   node scripts/add-favorito-field.mjs
 *
 * Requer:
...
const COLLECTION_NAME = "painelsorriso53_favoritos";
...
async function main() {
  ...
  console.log(`  Campos: usuario_id (text), paciente_id (text)`);
  console.log("");
  console.log("  Regras de acesso aplicadas:");
  console.log("  - List/View: somente registros do próprio usuário");
  console.log("  - Create: somente autenticado");
  console.log("  - Delete: somente próprios registros");
  console.log("");
  console.log("✓ Agora o sistema de favoritos per-user está pronto!");
  console.log("");
}
